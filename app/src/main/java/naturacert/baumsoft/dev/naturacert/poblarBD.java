...
public class poblarBD {
    ...
    public static void poblar(int referencia) {
        ...
        QueryBuilder qbR = DaoAPP.daoSession.getP1p1Dao().queryBuilder();
        qbR.where(p1p1Dao.Properties.Id.eq(1));
        List numero = qbR.list();

        if (numero.size() == 1) {
            Log.d("Informacion", "HAy un registro");
        } else {
            Log.d("Informacion", "Vamos a guardar el primer registro");
            ...
            op10p6.setObservaciones("9");
            DaoAPP.daoSession.getP10p6Dao().insert(op10p6);


        }

        /* revisamos si tiene rows para crear la primera
        QueryBuilder qb = DaoAPP.getPostDao().queryBuilder();
        qb.where(p1p1Dao.Properties.Id.eq(1));

        List<p1p1> posts = qb.list();
        //List<p1p1> posts = DaoAPP.getP1p1Dao().loadAll();
        for (p1p1 postr : posts) {

            Log.d("Sociales", String.valueOf(postr.getSociales()));
            Log.d("Laborales", String.valueOf(postr.getLaborales()));
            Log.d("Ambientales", String.valueOf(postr.getAmbientales()));

        }

        QueryBuilder qb2 = DaoAPP.getPostDao2().queryBuilder();
        qb2.where(p1p2Dao.Properties.Id.eq(1));

        List<p1p2> posts2 = qb2.list();
        //List<p1p1> posts = DaoAPP.getP1p1Dao().loadAll();
        for (p1p2 postr2 : posts2) {

            Log.d("Alp", String.valueOf(postr2.getAlp()));
            Log.d("Map", String.valueOf(postr2.getMap()));
            Log.d("Proc", String.valueOf(postr2.getProc()));
